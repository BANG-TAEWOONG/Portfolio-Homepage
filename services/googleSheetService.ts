...
const fallbackWorkData: WorkData = {
...
const parseCSVtoWorkData = (csvText: string): WorkData => {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // Fix: Type workItem as `any` to allow dynamic property assignment.
        const workItem: any = {};
        ...
        const typedWorkItem = {
            ...
            productionType: workItem.productionType,
        ...
        if (typedWorkItem.id && (typedWorkItem.productionType === 'produced' || typedWorkItem.productionType === 'participated')) {
            workData[typedWorkItem.id] = typedWorkItem as Work;
        }
    }
    return workData;
};
...
export const fetchWorkData = async (): Promise<WorkData> => {
    try {
